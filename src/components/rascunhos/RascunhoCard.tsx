...
export function RascunhoCard({ rascunho }: { rascunho: RascunhoComItens }) {
  ...
  return (
    <Card
      className={cn(
        "flex h-full flex-col gap-2 p-3 transition-shadow hover:shadow-md",
    ...
    >
      ...
      {rascunho.tipo === "texto" ? (
        <Textarea
          ...
          rows={5}
          disabled={!ehDono}
          className="flex-1 resize-none border-0 bg-transparent px-0 text-sm shadow-none focus-visible:ring-0"
        />
      ) : (
